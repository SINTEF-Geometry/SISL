...
#if defined(SISLNEEDPROTOTYPES)
void
sh6comedg_sf (SISLSurf * ps1, SISLSurf * ps2, SISLIntpt *pt1, SISLIntpt *pt2,
	      int *jstat)
#else
void
sh6comedg (ps1, ps2, pt1, pt2, jstat)
     SISLSurf *ps1;
     SISLSurf *ps2;
     SISLIntpt *pt1;
     SISLIntpt *pt2;
...
*
* PURPOSE    : Test if two intersection points is on a common edge and
*		connected along this edge on one of the surfaces.
*
*
* INPUT      : ps1      - Pointer to the first surface in the intersection.
*              ps2      - Pointer to the second surface in the intersection.
*              pt1      - Intersection point.
*              pt2      - Intersection point.
*
*
...
*                        < 0  : Error.
*			   0  : Not on a common edge.
*			   1  : On a common edge in first surface.
*			   2  : On a common edge in second surface.
*			   3  : On a common edge in bouth surfaces.
*
*
* METHOD     :
*
...
* REFERENCES :
*
* WRITTEN BY : Vibeke Skytt, SINTEF, 052023
*********************************************************************
*/
{
 int kstat=0;
 ...
 *jstat = 0;
 ...
 if (pt1 != SISL_NULL && pt2 != SISL_NULL)
 {
   ...
   minpar[0] = ps1->et1[ps1->ik1-1];
   minpar[1] = ps1->et2[ps1->ik2-1];
   maxpar[0] = ps1->et1[ps1->in1];
   maxpar[1] = ps1->et2[ps1->in2];

   minpar[nrpar]   = ps2->et1[ps2->ik1-1];
   minpar[nrpar+1] = ps2->et2[ps2->ik2-1];
   maxpar[nrpar]   = ps2->et1[ps2->in1];
   maxpar[nrpar+1] = ps2->et2[ps2->in2];
   nrpar += 2;
   np2 = 4;
